...
def create_run_id(spec: FeatureSpec, audit_root: Path = AUDIT_ROOT) -> str:
...
def get_run_dir(run_id: str, audit_root: Path = AUDIT_ROOT) -> Path:
    if "/" in run_id or "\\" in run_id or ".." in run_id:
        raise PipelineError(f"invalid run id: {run_id}")
    return audit_root / run_id


def require_run_dir(run_id: str, audit_root: Path = AUDIT_ROOT) -> Path:
...
def initialize_run(spec: FeatureSpec, spec_file: Path, audit_root: Path = AUDIT_ROOT) -> tuple[str, Path, str]:
